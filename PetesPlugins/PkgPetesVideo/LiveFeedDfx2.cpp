...
class CLiveFeed2Dfx: public CDFX
{
	DECLARE_EFFECT()
...
_screen_info CLiveFeed2Dfx::m_InputScreensInfo[1];
INT CLiveFeed2Dfx::m_InputScreensCount=0;

BEGIN_OUTPUTSCREENS_DESC(CLiveFeed2Dfx)
{-1,-1},  ""
END_OUTPUTSCREENS_DESC(CLiveFeed2Dfx)

BEGIN_EFFECT_DESC(CLiveFeed2Dfx, 11213216851273229, PETE_VJO_VERSION , "PetesVideo\\LiveFeed3", "Pete Warden", "www.petewarden.com")
END_EFFECT_DESC(CLiveFeed2Dfx)

REGISTER_DFXFACTORY(CLiveFeed2Dfx)

CLiveFeed2Dfx::CLiveFeed2Dfx()
{
	m_nXRes = 0;
	m_nYRes = 0;
	...
	m_nMemUsage = sizeof(CLiveFeed2Dfx);
}
...
HBITMAP	CLiveFeed2Dfx::GetLabelBitmap()
{
	return (HBITMAP)HBITMAP_LAYERS;
}

BOOL	CLiveFeed2Dfx::SetupDialog()
{
	VJO_MANAGE_STATE();
...
void	CLiveFeed2Dfx::SetConfigData(CConfigData *p)
{
...
BOOL	CLiveFeed2Dfx::GetConfigData(CConfigData *p)
{
	ASSERT(p);
...
BOOL	CLiveFeed2Dfx::UpdateConfig()
{
...
BOOL	CLiveFeed2Dfx::Initialize()
{
	SIZE sz = m_pEngine->GetOutputRes();
	m_nXRes = sz.cx;
	m_nYRes = sz.cy;

	Pete_LiveFeed_Init(&m_InstanceData,m_nXRes,m_nYRes,2);

	return TRUE;
}

BOOL	CLiveFeed2Dfx::Render(CScreen **ppInput, CScreen *pOutput)
{
...
DWORD	CLiveFeed2Dfx::GetMemoryUsage()
{
	return m_nMemUsage;
}

BOOL CLiveFeed2Dfx::ConfirmRenderSource(int i)
{
	return TRUE;
}

DWORD CLiveFeed2Dfx::GetInputState()
{
	return 0xffffffff;
}
