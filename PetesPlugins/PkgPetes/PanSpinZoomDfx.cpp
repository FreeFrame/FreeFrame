...
class CPanSpinZoomDfx: public CDFX
{
	DECLARE_EFFECT()
...
protected:
	...
	SPete_PanSpinZoom_Data m_InstanceData;
	SPete_PanSpinZoom_Settings m_Settings;
	SPete_Parameter* m_pParameters;
	int		m_nParameterCount;

};

BEGIN_INPUTSCREENS_DESC(CPanSpinZoomDfx)
{{-1,-1}, "RGB 0"},
END_INPUTSCREENS_DESC(CPanSpinZoomDfx)

BEGIN_OUTPUTSCREENS_DESC(CPanSpinZoomDfx)
{-1,-1},  ""
...
CPanSpinZoomDfx::CPanSpinZoomDfx()
{
	m_nXRes = 0;
	m_nYRes = 0;
	...
	Pete_PanSpinZoom_GetParameters(&m_pParameters,&m_nParameterCount);
	...
	m_nMemUsage = sizeof(CPanSpinZoomDfx);
}

CPanSpinZoomDfx::~CPanSpinZoomDfx()
{

	Pete_PanSpinZoom_DeInit(&m_InstanceData);
...
BOOL	CPanSpinZoomDfx::SetupDialog()
{
	VJO_MANAGE_STATE();
...
void	CPanSpinZoomDfx::SetConfigData(CConfigData *p)
{
...
BOOL	CPanSpinZoomDfx::GetConfigData(CConfigData *p)
{
	ASSERT(p);
...
BOOL	CPanSpinZoomDfx::Initialize()
{
	SIZE sz = m_pEngine->GetOutputRes();
	m_nXRes = sz.cx;
	m_nYRes = sz.cy;

	Pete_PanSpinZoom_Init(&m_InstanceData,m_nXRes,m_nYRes);
...
BOOL	CPanSpinZoomDfx::Render(CScreen **ppInput, CScreen *pOutput)
{
	...
	Pete_PanSpinZoom_Render(&m_InstanceData,&m_Settings,pInputMem,pOutputMem);

	return TRUE;

}

DWORD	CPanSpinZoomDfx::GetMemoryUsage()
{
	return m_nMemUsage;
}

BOOL CPanSpinZoomDfx::ConfirmRenderSource(int i)
{
	return TRUE;
}

DWORD CPanSpinZoomDfx::GetInputState()
{
	return 0xffffffff;
}
